...
- Appearance…: opens a window with two sections. "Menu bar text" is a template field for the menu bar's label, built from `{5h}`, `{week}`, `{reset5h}`, `{resetWeek}` and `{cost}` (an empty field draws the icon alone; a live preview shows what the current tab would read); the default is ` {5h} / {week}`, the app's original layout. Below it, the colour scale editor gives each of the four bands its own colour picker and the three cutoffs between them can be moved (1 to 100, kept in order). Changes apply at once to the bars on both tabs and to the menu bar icon; the Auto tint rule follows the High cutoff. "Reset to defaults" puts the original scale back.
