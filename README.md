...
- Usage colours…: opens a window where each of the four bands gets its own colour picker and the three cutoffs between them can be moved (1 to 100, kept in order). Changes apply at once to the bars on both tabs and to the menu bar icon; the Auto tint rule follows the High cutoff. "Reset to defaults" puts the original scale back.
